(ns tensorrt.main
  "Kotodama WASM entrypoint for the Tensorrt clean-room actor (L4) — Clojure port.
  ...
  facts (`tensorrt.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tensorrt")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tensorrt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tensorrt-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tensorrt-compat/ready)
